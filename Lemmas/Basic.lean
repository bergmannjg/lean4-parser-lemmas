...
* apply a parser that respects the position
* reset to the saved position
 -/
class Stream.RespectsPosition (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]  where
  ...
  setPosition_of_getPosition_eq (s1 s2 : σ) (p : Position σ)
      : Stream.getPosition s1 = p  → r s1 s2 → Stream.setPosition s2 p = s1
...
abbrev Stream.respectsPosition [Parser.Stream σ τ] [Stream.Remaining σ]
    [Stream.RespectsPosition σ τ] (s1 s2 : σ) : Prop :=
...
def respectsPosition (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
  [Stream.RespectsPosition σ τ] (p : SimpleParser σ τ α)
  := ∀ it rem, (match (p it) with | .ok rem _ => rem | .error rem _ => rem) = rem
...
def notIncrementsRemainingOnSuccess (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
   (p : SimpleParser σ τ α)
  := ∀ it rem a, p it = .ok rem a → Stream.notIncrementsRemaining it rem
...
def decrementsRemainingOnSuccess (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
   (p : SimpleParser σ τ α)
  := ∀ it rem a, p it = .ok rem a → Stream.decrementsRemaining it rem

/-- Defines the postcondition of Std.Stream.next? on a non empty stream with class Stream.RespectsPosition
 -/
class Stream.Next?OnInput (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
...
class Stream.Next?OnEndOfInput (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
   where
  cond (it : σ) : (0 = Stream.Remaining.remaining it) → Std.Stream.next? it = none

/-- Defines a precondition for Stream.setPosition, so that Parser.setPosition gives a valid result
 -/
class Stream.SetPositionPrecondition (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
  [Stream.RespectsPosition σ τ] where
  ...
  of_getPosition s1 s2 p : Stream.getPosition s1 = p → Stream.respectsPosition s1 s2 → cond s2 p

/-- example with decrementsRemainingOnSuccess to prove termination -/
def foldr'  (σ τ : Type) [Parser.Stream σ τ] [Stream.Remaining σ]
  (f : α → β → β) (p : SimpleParser σ τ α) (q : SimpleParser σ τ β)
